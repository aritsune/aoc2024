        ...
        match rules_map.get(num) {
            ...
            Some(not_allowed) => {
                for prevnum in previous {
                    if not_allowed.contains(prevnum) { return false; }
                }
            }
        }
    }
...
pub fn solve(data: &str) {
    ...
    for (i, ordline) in data.lines().enumerate() {
        ...
        let before: u8 = left.parse().expect("left side number to be parseable into u8");
        let after: u8 = right[1..].parse().expect("right side number to be parseable into u8");
    ...
    for updateline in data.lines().skip(split + 1) {
        if updateline.is_empty() { continue }
        let updateset: Vec<u8> = updateline.split(',').map(|strnum| strnum.parse::<u8>().expect("all numbers in input data to be parseable")).collect();
        if set_is_valid(&updateset, &rules_map) {
            valid_sets.push(updateset);
        } 
    }
    let answer: usize = valid_sets.into_iter().map(|set| usize::from(set[set.len() / 2])).sum();
    ...
    for updateline in data.lines().skip(split + 1) {
        ...
        if set_is_valid(&updateset, &rules_map) { continue };
        while !set_is_valid(&updateset, &rules_map) {
            updateset = fix_set(updateset, &rules_map);
        }
        fixed_sets.push(updateset);
    }
    let answer_two: usize = fixed_sets.into_iter().map(|set| usize::from(set[set.len() / 2])).sum();
    println!("Total of middle numbers of fixed incorrect sets is {}", answer_two);
}
